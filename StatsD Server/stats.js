...
//  if (config.debug) {
    console.log("Loading server: " + name, 'DEBUG');
//  }
  var port = 8125;
  var ret = servermod.start(config, callback, port);
  console.log("Listening on: " + port);
  if (!ret) {
    console.log("Failed to load server: " + name, "ERROR");
    process.exit(1);
  }
}
...
config.configFile(process.argv[2], function (config) {
  ...
  if (!serversLoaded) {
    ...
    if (config.backends) {
      ...
      loadBackend(config, './backends/ruban-monitor-console');
    }
